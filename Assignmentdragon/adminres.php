...
<?PHP
	...
	$url = "adminres.php?day=" . $day . "&month=" . $month . "&year=" . $year;
...
<html>
    ...
    <nav>
      <ul>
		...
		<div id="controls">
			<a href="adminmonthview.php?year=<?PHP echo $year; ?>&month=<?PHP echo $month;?>">Back to Monthly View</a>
		</div>	
						...
						for ($i = 0; $i < 7; $i++) {
							...
							if ($result->num_rows > 0) {
								...
								foreach($result as $row) {								
									echo $row['ForeName'] . " " . $row['SurName'] . "<br/>" . $row['Email'] . "<br/>" . $row['Phone'];
								}
							} else {
