...
<script src="http://use.edgefonts.net/source-sans-pro:n2:default.js" type="text/javascript"></script>
    ...
    <nav>
      <ul>
		...
		<div id="controls">
			<a href="managecmr.php">Manage Customers</a>
		</div>
		...
		<table id="monthview">
			...
			<?PHP 
				...
				for ($i = 1; $i <= $daysinmonth; $i++) {
					...
					if ($dayindex <= 5) echo '<td class="day"><a href="adminres.php?day=' . $i . '&month=' . $month . '&year=' . $year . '">' . $i . '</a></td>';
					else echo '<td class="weekend"><a href="adminres.php?day=' . $i . '&month=' . $month . '&year=' . $year . '">' . $i . '</a></td>';
		...
		<div id="nextback">
			<a href="adminmonthview.php?year=<?PHP echo $lastyear; ?>&month=<?PHP echo $lastmonth;?>">Previous</a>
			<a href="adminmonthview.php?year=<?PHP echo $nextyear; ?>&month=<?PHP echo $nextmonth;?>">Next</a>
		</div>
		
		<div id="jumptomonth">
			<form action="adminmonthview.php" method="GET" id="directinputmonth">
				<select name="month">
					<?PHP
						// format month names
